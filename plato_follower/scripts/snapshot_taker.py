...
class RADLOCCSnapshotter(object):
    ...
    def __init__(self):
        ...
        self.camera_topic = rospy.get_param("~camera_topic",
                                            '/global_shutter_mono_camera_throttled/image_raw')
        self.output_directory = rospy.get_param(
            "~output_directory", 'camera_laser_snapshots')
        self.range_cutoff = rospy.get_param("~range_cutoff", 2.5)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if "q" in input_str:
                break
            try:
                laser_msg = rospy.wait_for_message(
                    self.scan_topic, LaserScan, timeout=0.25)
            except rospy.exceptions.ROSException as err:
                print "Failed to receive laser message: " + str(err)
                continue
            try:
                camera_msg = rospy.wait_for_message(
                    self.camera_topic, Image, timeout=0.25)
            except rospy.exceptions.ROSException as err:
